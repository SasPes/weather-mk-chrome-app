...
function weatherForecast() {
	...
	var feed = new google.feeds.Feed("http://weather.yahooapis.com/forecastrss?w=" + gradIdF + "&u=c");
    feed.setNumEntries(1);
...
function parsWeatherForecast(day) {
	...
	var startH = day.indexOf("High: ");
	startH = startH + "High: ".length;
	var endH = day.indexOf(" Low: ");
	var visoka = day.substring(startH, endH);
	
	var startL = endH + " Low: ".length;
	var endL = day.indexOf("<br>");
	var niska = day.substring(startL, endL);
	
	var endS = day.indexOf(". High: ");
	var slika = day.substring(6, endS);
	var slikaNum = "na";
	if(slika == "Thunderstorms Early"){
		...
		slikaNum = 30;
	} else if(slika == "Scattered T-Storms" || slika == "PM T-Storms"){
		slikaNum = 38;
